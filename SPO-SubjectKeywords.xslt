...
<!-- 
    ...
    2. Open a command prompt using Start > Run and type cmd
      ...
      c:\msxsl.exe c:\<yyyymmdd>-RDM_Export.xml c:\SPO-SubjectKeywords.xslt -o c:\<yyyymmdd>-SubjectKeywords_RDM.xml
  -->
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:hiq="http://vpsl.co.uk" exclude-result-prefixes="hiq">
  ...
  <xsl:variable name="TopLevelNodes" select="/hiq:CVRObjects/hiq:MODXMLbodyStructure/hiq:CVRObject[@name and not(hiq:ObjectRelationships/hiq:ObjectRelationshipItem/hiq:ObjectRelationshipType/@name='Non Preferred Term For') and not(hiq:ObjectRelationships/hiq:ObjectRelationshipItem/hiq:ObjectRelationshipType/@name='Child Of') and (hiq:ObjectRelationships/hiq:ObjectRelationshipItem/hiq:ObjectRelationshipType/@name='Parent Of')]" />
  <xsl:template match="/">
    <TermStores>
      <TermStore Name="" IsOnline="" WorkingLanguage="" DefaultLanguage="" SystemGroup="">
        <Groups>
          <Group Id="TRANSFORMED" Name="Subject Keywords (UK Defence Thesaurus)" Description="" IsSystemGroup="" IsSiteCollectionGroup="">
            <TermSets>
              <TermSet Id="" Name="Subject Keywords" Description="Use Subject Keywords to describe as specifically as possible, the document content. Selected from the UK Defence Thesaurus." Contact="" IsAvailableForTagging="True" IsOpenForTermCreation="False" CustomSortOrder="" TermCount="">
                <CustomProperties>
                  ...
                  <xsl:for-each select="$TopLevelNodes">
                    ...
                    <Term Name="{@name}" Id="" CustomSortOrder="" IsAvailableForTagging="" IsSourceTerm="" IsRoot="" IsReused="" IsKeyword="" IsDeprecated="" CVRObjectID="{@id}" Processed="False">
                      ...
                      <LocalCustomProperties>
                        ...
                        <!-- Loop around all the "See Also" nodes -->
                        <xsl:for-each select="hiq:ObjectRelationships/hiq:ObjectRelationshipItem[hiq:ObjectRelationshipType/@name='See Also']">
                          <xsl:variable name="SACounter" select="position()-1"/>
                          <xsl:choose>
                            <xsl:when test="$SACounter=0">
                              <LocalCustomProperty Key="See Also" Value="{hiq:ObjectRelationshipObject/@id}"/>
                            </xsl:when>
                            <xsl:otherwise>
                              <LocalCustomProperty Key="See Also{$SACounter}" Value="{hiq:ObjectRelationshipObject/@id}"/>
                            </xsl:otherwise>
                          </xsl:choose>
                        </xsl:for-each>
                      </LocalCustomProperties>
  ...
  <xsl:template name="node_children">
    ...
    <Term Id="" Name="{$node_name}" IsDeprecated="" IsAvailableForTagging="" IsKeyword="" IsReused="" IsRoot="" IsSourceTerm="" CustomSortOrder="" CVRObjectID="{$node_id}" Processed="False">
      ...
      <LocalCustomProperties>
        ...
        <!-- Loop around all the "See Also" nodes -->
        <xsl:for-each select="key('object_key',$node_id)/hiq:ObjectRelationships/hiq:ObjectRelationshipItem[hiq:ObjectRelationshipType/@name='See Also']">
          <xsl:variable name="SACounter" select="position()-1"/>
          <xsl:choose>
            <xsl:when test="$SACounter=0">
              <LocalCustomProperty Key="See Also" Value="{hiq:ObjectRelationshipObject/@id}"/>
